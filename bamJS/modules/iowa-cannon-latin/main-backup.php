<?php
   $PageTitle="Iowa Canon of Latin Authors";
   $NavBarBrand ="Iowa Canon of Latin Authors";
...
?>
    ...
    <body>
        ...
        <script>
            //disabled for now -  this is a parser for perio.do data. The idea is to have data that is formatted to use perio.do URIs in place of actual centuries when possible
            /*
                    var timeDefinitions = [];
                    $.getJSON("periodo-BAM_Periods.json", function(data) {
                        //this iterates over the perio.do .json structure to get period definitions	
                        for (var key in data.periodCollections) {
                            if (data.periodCollections.hasOwnProperty(key)) {
                                for (var subKey in data.periodCollections[key]) {
                                    if (data.periodCollections[key].hasOwnProperty(subKey)) {
                                        if (subKey === 'definitions') {
                                            //setup the period definitions
                                            timeDefinitions = data.periodCollections[key][subKey];
                                            console.log(timeDefinitions);
                                        }
            ...
            */
            ...
            var svg = d3.select("#networkpane")
                ...
                .attr("viewBox", "-3500 -1100 " + width * 40 + " " + height * 40)
            ...
            d3.json("iclaw.json", function(error, json) {
                ...
                function connectedNodesList(d) {
                        ...
                        if (neighboring(d, o) | neighboring(o, d) == 1) {
                            ...
                            if (o.label != d.label) {
                                ...
                                if (o.attributes.type == 'place') {
                                    if (placesIdForNoDuplicates.indexOf(o.id) == -1) {
                                        mapConnectionsHolder.push(placesList[o.id]);
                                        placesIdForNoDuplicates.push(o.id);
                                    }
                                }
                    ...
                    console.log(peopleHolder);

                //maybe could make all of this a singular function?

                $('#networkLauncher').click(function() {
                    $('#infoBox').hide();
                    $('#networkBox').show();
                });
                ...
                $('#networkBoxCloseBox').mousedown(function() {
                    $('#networkBox').hide();
                });
                ...
                function onEachFeature(feature, layer) {
                    ...
                    layer.on('click', function(e) {
                        ...
                        var titleForBox = e.target.feature.properties.label;
                        var htmlForBox = titleForBox;
                        $("#leftTextPaneContent").html(htmlForBox);
                    });
                }

                // console.log(placesHolder);
                ...
                //calling the layers as a separate object
                var layers = {
                ...
                work_locations.eachLayer(function(layer) {
                ...
                function changeMapFromTimeLine(map, inputLayer, start, end, placesHolder) {
                    ...
                    for (var i = 0; i < masterPlacesList.length; i++) {
                        masterPlacesList[i].properties.count = '1';
                        var tempYearStart = masterPlacesList[i].properties.start.split('/');
                        var tempYearEnd = masterPlacesList[i].properties.end.split('/');
                        var yearStart = tempYearStart[2];
                        var yearEnd = tempYearEnd[2];

                        //now check to see if the item is within the year parameters
                        if (yearStart >= start && yearEnd <= end) {
                            if (inArray(masterPlacesList[i].properties.id, placesHolder) == false) {
                    ...
                    for (var i = 0; i < placesHolder.length; i++) {
                        inputLayer.addData(placesHolder[i]);
                    }
                ...
                $('#timeStart').on('input propertychange change keyup paste', function() {
                    changeMapFromTimeLine(map, work_locations, $('#timeStart').val(), $('#timeEnd').val(), placesHolder)
                });
