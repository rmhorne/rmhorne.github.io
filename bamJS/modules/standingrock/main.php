<?php
   $PageTitle="Standing Rock Twitter Map";
   $NavBarBrand ="Standing Rock Twitter Mention Map</i>";
    ...
    minWidth:300
            ...
            d3.json("basic-map.json", function(error, json) {
                ...
                var nodes = json.nodes.map(function(d) {
                    ...
                    if (isNaN(d.attributes.lon) == false) {

                        //this should be some kind of config, but different projects may wish to show different data
                        var geojsonFeature = {
                            'type': 'Feature',
                            'properties': {
                                'id': d.attributes.id_str,
                                'label': d.attributes.id_str,
                                'from_user': d.attributes.from_user,
                                'status_url': d.attributes.status_url,
                            ...
                            'geometry': {
                                'type': 'Point',
                                'coordinates': [d.attributes.lat, d.attributes.lon]
                            }
                        };
            ...
            var layers = {
                ISAW: L.mapbox.tileLayer('isawnyu.map-p75u7mnj', {                }),
                Empty: new L.tileLayer('')
            };

            layers.ISAW.addTo(map);
...
//modify the information in the popup as needed
            function makeBamPopup(feature, map) {
                ...
                var htmlForBox = '<div id="first-tweet"></div><center><table><tr>';
